...
type(dada_forward)
dada_forward$`SRR15570324_1_filtered.fastq.gz`
...
chim_check <- sum(seqtab_nochim)/sum(seqtab) # 10.00414
...
track %>%
  ...
  facet_grid(~location) + 
  geom_line(aes(group = Run), color = "grey") + 
...
mock_sample <- seqtab_nochim["SRR15570324",]
length(mock_sample)
...
# Read in metadata 
my_metadata = 
  read.csv("/workdir/jd2263/BIOMI6300_Project2_AmpliconAnalysis/data/metadata.csv") %>%
  mutate(X = NULL) %>%
  # fix typo 
...
#rownames(my_metadata) <- metadata$names
```
...
raw_physeq
save(raw_physeq, file = paste0("/local/workdir/jd2263/BIOMI6300_Project2_AmpliconAnalysis/data/raw_physeq.RData"))
```
